...
# === NEW HELPER FUNCTION TO HANDLE QUOTES ===
def get_env_variable(key):
    """Gets an environment variable and strips surrounding quotes if they exist."""
    value = os.getenv(key)
    if value:
        # Strips both ' and " from the beginning and end of the string
        return value.strip('\'"')
    return None
...
def save_anomaly_clip(frames):
    """Saves a batch of frames as a video file."""
    if not frames:
        return None
